...
func _end_tui(session:TuiSession) -> void:
	...
	_tui_hidden_controls.clear()
	if not _leaving_tree and is_visible_in_tree():
		focus_input()
...
func update_prompt() -> void:
	if _manual_prompt:
		...
		_prompt_bbcode = str(prompt_formatter.call(context))
		_prompt_changed()
	else:
...
func _render_prompt(text:String, color:Color) -> void:
	_prompt_bbcode = text if color == Color.WHITE else \
...
func _apply_font_override(_font:Font) -> void:
	pass
...
func _set_input_editable(_editable:bool) -> void:
	pass
...
func _set_input_text(_value:String) -> void:
	pass
